...
Sample broker for for Akri's [end to end demo](https://docs.akri.sh/demos/usb-camera-demo).
It pulls video frames from the USB camera with device node specified by the
environment variable named with `UDEV_DEVNODE_<6 digit digest>`.
Then, it serves these frames over a gRPC interface.
The [streaming application](../../apps/video-streaming-app/) provides an example
